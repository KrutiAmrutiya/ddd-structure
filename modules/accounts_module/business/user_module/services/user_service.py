...
class UserService:
        ...
        q = (
            self.Query.into(table)
                ...
                .insert(data.username, data.first_name, data.last_name, data.password, data.email,
                        data.gender, data.phone_number, data.date_of_birth, True, True, True,
                        "1999-05-01 05:30:00+05:30")
        )
    ...
    def login(self, data: UserDto) -> Set[str]:
        """[summary]
        ...
        """
        table = self.Table("accounts_user")
        # q = self.Query.from_(table).select("*").where(table.username == data.username).where(table.password ==
        # data.password)
